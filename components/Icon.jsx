const paths = {
  share: 'M13 5v6.5M13 5l-3 3M13 5l3 3M6 12v6a2 2 0 002 2h10a2 2 0 002-2v-6',
  camera: 'M4 8h3l1.5-2h7L17 8h3a1 1 0 011 1v9a1 1 0 01-1 1H4a1 1 0 01-1-1V9a1 1 0 011-1zm8 3.5a3.5 3.5 0 100 7 3.5 3.5 0 000-7z',
  target: 'M12 3v3m0 12v3M3 12h3m12 0h3M12 7.5a4.5 4.5 0 100 9 4.5 4.5 0 000-9z',
  browser: 'M3 6a2 2 0 012-2h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V6zm0 3.5h18M6 7h.01M8.5 7h.01',
  code: 'M9 8l-4 4 4 4m6-8l4 4-4 4',
  bolt: 'M13 3L5 13h5l-1 8 8-10h-5l1-8z',
  cup: 'M5 8h11v6a5 5 0 01-5 5H10a5 5 0 01-5-5V8zm11 1h2a2 2 0 010 4h-2M4 21h13',
  sparkle: 'M12 3l1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8L12 3zM18.5 15.5l.8 2.2 2.2.8-2.2.8-.8 2.2-.8-2.2-2.2-.8 2.2-.8.8-2.2z',
  building: 'M4 21V6l7-3 7 3v15M4 21h16M9 9h.01M13 9h.01M9 13h.01M13 13h.01M9.5 21v-4h3v4',
  heart: 'M12 20s-7-4.4-7-9a4 4 0 017-2.6A4 4 0 0119 11c0 4.6-7 9-7 9z',
  cap: 'M3 9l9-4 9 4-9 4-9-4zm3 3.5V17c0 1.5 2.7 3 6 3s6-1.5 6-3v-4.5',
  bag: 'M6 8h12l1 12H5L6 8zm3 0V6a3 3 0 016 0v2',
  box: 'M12 3l8 4.2v9.6L12 21l-8-4.2V7.2L12 3zm0 0v18m8-13.8L12 12 4 7.2',
  brief: 'M4 8h16v11a1 1 0 01-1 1H5a1 1 0 01-1-1V8zm5 0V6a2 2 0 012-2h2a2 2 0 012 2v2M4 13h16',
  check: 'M4.5 12.5l5 5 10-11',
  arrow: 'M5 12h14m-6-6l6 6-6 6',
  plus: 'M12 5v14M5 12h14',
  phone: 'M5 4h4l2 5-2.5 1.5a11 11 0 005 5L15 13l5 2v4a1 1 0 01-1 1A16 16 0 014 5a1 1 0 011-1z',
  mail: 'M3 6h18v12H3V6zm0 0l9 7 9-7',
  pin: 'M12 21s7-5.6 7-11a7 7 0 10-14 0c0 5.4 7 11 7 11zm0-8.5a2.5 2.5 0 100-5 2.5 2.5 0 000 5z',
  wa: 'M4 20l1.3-4A8 8 0 1112 20a8 8 0 01-4-1.1L4 20z',
  instagram: 'M7 3h10a4 4 0 014 4v10a4 4 0 01-4 4H7a4 4 0 01-4-4V7a4 4 0 014-4zM12 8.2a3.8 3.8 0 100 7.6 3.8 3.8 0 000-7.6zM16.6 6.4h.01',
  linkedin: 'M7 3h10a4 4 0 014 4v10a4 4 0 01-4 4H7a4 4 0 01-4-4V7a4 4 0 014-4zM7.5 7.8h.01M7.5 10.5V17M11 17v-4a2 2 0 014 0v4M11 17v-6.5',
  chevron: 'M6 9l6 6 6-6',
  search: 'M11 4a7 7 0 100 14 7 7 0 000-14zM21 21l-4.35-4.35',
  shield: 'M12 3l7 3v6c0 4.5-3 8-7 9-4-1-7-4.5-7-9V6l7-3z',
  play: 'M8 5l11 7-11 7V5z',
  pencil: 'M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z',
  chart: 'M6 20V10M12 20V4M18 20v-6',
  megaphone: 'M3 10v4a1 1 0 001 1h2l10 5V4L6 9H4a1 1 0 00-1 1zM15 8a4 4 0 010 8M18.5 5a8 8 0 010 14',
  facebook: 'M14 21v-7h2.5l.5-3H14V9a1.5 1.5 0 011.5-1.5H17V4.5h-2A4 4 0 0011 8.5V11H9v3h2v7',
  bot: 'M12 3v3M7 8h10a2 2 0 012 2v7a2 2 0 01-2 2H7a2 2 0 01-2-2v-7a2 2 0 012-2zM9 13v.01M15 13v.01M9 17h6',
  cursor: 'M6 4l14 6.2-6 1.8-1.8 6L6 4z',
  people: 'M8 12a3 3 0 100-6 3 3 0 000 6zM2 20c0-3.3 2.7-6 6-6s6 2.7 6 6M16 8a3 3 0 100-6M22 20c0-2.8-1.8-5.1-4.5-5.8',
  trend: 'M3 17l6-6 4 4 8-8M15 7h6v6',
  chat: 'M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z',
  document: 'M14 3H6a2 2 0 00-2 2v14a2 2 0 002 2h12a2 2 0 002-2V9zM14 3v6h6M9 13h6M9 17h6M9 9h1',
  userPlus: 'M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8M20 8v6M23 11h-6',
  send: 'M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z',
  pie: 'M21.21 15.89A10 10 0 118 2.83M22 12A10 10 0 0012 2v10z',
  qr: 'M4 4h6v6H4V4zm2 2v2h2V6H6zM14 4h6v6h-6V4zm2 2v2h2V6h-2zM4 14h6v6H4v-6zm2 2v2h2v-2H6zM14 14h2v2h-2v-2zM18 14h2v2h-2v-2zM14 18h2v2h-2v-2zM18 18h2v2h-2v-2z',
  percent: 'M6 18L18 6M7.5 9a1.5 1.5 0 100-3 1.5 1.5 0 000 3zM16.5 18a1.5 1.5 0 100-3 1.5 1.5 0 000 3z',
  image: 'M4 5h16a1 1 0 011 1v12a1 1 0 01-1 1H4a1 1 0 01-1-1V6a1 1 0 011-1zm3 3.5a1.5 1.5 0 100 3 1.5 1.5 0 000-3zM3.5 16l4.5-4.5 3 3 3.5-3.5 5.5 5.5',
  braces: 'M8 4.5c-2 0-3 1-3 3v2.5c0 1.2-.9 2-2 2 1.1 0 2 .8 2 2V17c0 2 1 3 3 3M16 4.5c2 0 3 1 3 3v2.5c0 1.2.9 2 2 2-1.1 0-2 .8-2 2V17c0 2-1 3-3 3',
  key: 'M14.5 3a5.5 5.5 0 105.14 7.5H21v3h-3v3h-3v-2.15A5.5 5.5 0 0114.5 3zm0 3a2.5 2.5 0 100 5 2.5 2.5 0 000-5zM9 15L3 21m6-6l2 2m-2-2l-2 2',
  lock: 'M6 11V8a6 6 0 1112 0v3M5 11h14a1 1 0 011 1v8a1 1 0 01-1 1H5a1 1 0 01-1-1v-8a1 1 0 011-1zM12 15v3',
  rocket: 'M12 2c2.4 2.1 3.8 5.4 3.8 8.8 0 2-.5 3.6-1.5 5.1L12 19l-2.3-3.1c-1-1.5-1.5-3.1-1.5-5.1C8.2 7.4 9.6 4.1 12 2zM9 14.5l-2.5 1.3L6 19l3.3-2M15 14.5l2.5 1.3.5 3.2-3.3-2M12 8.7a1.4 1.4 0 100 2.8 1.4 1.4 0 000-2.8z',
};

export default function Icon({ name, className = 'h-5 w-5', strokeWidth = 1.7 }) {
  const d = paths[name] || paths.check;
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      aria-hidden="true"
    >
      <path d={d} />
    </svg>
  );
}
